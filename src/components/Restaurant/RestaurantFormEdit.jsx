...
const RestaurantForm = () => {
  ...
  useEffect(() => {
    axios
      ...
      .then((response) => {
        ...
        setJOuverture(restaurantData.jour_ouverture);
        setHFermeture(restaurantData.heure_fermeture);
        setHOuverture(restaurantData.heure_ouverture);
        setLatitude(restaurantData.latitude);
        setLongtitude(restaurantData.longitude);
        setNom(restaurantData.nom);
        setRank(restaurantData.rank);
        setVilleId(restaurantData.zone.ville.id_Ville );
        setSerieId(restaurantData.serie.id_Serie );
        setZoneId(restaurantData.zone.id_Zone );
      })
      .catch((error) => {
      });
  ...
  return (
    <div className="container">
      <h2 className="mt-5">Edit Restaurant</h2>
      <form onSubmit={handleSubmit}>
        ...
        <div className="form-group">
          ...
          >
            ...
            {dataz.map((item) => (
              <option key={item.id_Zone} value={item.id_Zone} selected={item.id_Zone === zoneId}>
                {item.nom}
              </option>
            ))}
          </select>
        ...
        <div className="form-group">
          ...
          >
            ...
            {datas.map((item) => (
              <option key={item.id_Serie} value={item.id_Serie} selected={item.id_Serie === serieId}>
                {item.nom}
              </option>
            ))}
          </select>
